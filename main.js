...
import { renderTiles, renderRoads, clearDecorOnTile, populateDecorModels } from './systems/renderWorld.js';
...
function updateDayNightVisual(dt) {
  ...
  sceneCtx.sun.intensity = clamp(Math.sin(ang) * 1.08 + 1.12, .8, 2.1) * lightMul;
  sceneCtx.hemi.intensity = .9 + sceneCtx.sun.intensity * .38;
  sceneCtx.ambient.intensity = .5 + sceneCtx.sun.intensity * .09;
  if (sceneCtx.fill) sceneCtx.fill.intensity = .4 + sceneCtx.sun.intensity * .12;
